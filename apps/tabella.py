...
DATA_PATH = "\\\\servercorse\\Ducaticorse\\AER - Sviluppo e Progettazione Aerodinamica\\CFD\\_DC\\GP"


directory_all=os.listdir(DATA_PATH)
directory=[]
...
##### CREO DATAFRAME COMPLETO CON CARICHI ######
def loop(df2):
    for ii in df['Directory']:
        ...
        for ll in l:
            k=os.listdir(ii+'/'+ll)
            for kk in k:
                if kk.startswith('DES'):
                    p=os.listdir(ii+'/'+ll+'/'+kk)
                    for pp in p:
                        if pp.startswith('loads'):
                            fname2=ii+'/'+ll+'/'+kk+'/'+pp
                            if open(fname2): 
                                with open(fname2) as ff:
                                    line = ff.readlines()
                                    for aa in range( 0, len(line)):
                                        splitt = line[aa].split(" = ",1)
                                        string = splitt[0]
                                        val = float(splitt[1])
                                        df2.loc[(df['Directory']==ii) & (df['Case']==ll),string]=val
                                        return df2
            ...
            if kk.startswith('MP'):
                ...
                for pp in p:
                    if pp.startswith('report_'):
                        df.loc[(df['Directory']==ii) & (df['Case']==ll),'Report']=DATA_PATH+"/"+ii+'/'+ll+'/'+kk+'/'+pp
                        #print (ii,ll,kk,pp)
                    if pp.startswith('loads'):
                        dataz = pd.read_csv(ii+'/'+ll+'/'+kk+'/'+pp, header = None)
                        fname2=ii+'/'+ll+'/'+kk+'/'+pp
                        with open(fname2) as ff:
                            line = ff.readlines()
                            lenght=len(line)
                            for aa in range( 0, len(line)):
                                splitt = line[aa].split(" = ",1)
                                string = splitt[0]
                                val = "ok" #float(splitt[1])
                                df.loc[(df['Directory']==ii) & (df['Case']==ll), string]=val
